...
#        }
        ...
        ### updated llostic2.R function in package
        if (type=="relative"){

        EDder <-
            c(-log(exp(-tempVal/parmVec[5])-1)/(parmVec[1]^2),
              0, 0, 1/parmVec[4],
              exp(-tempVal/parmVec[5])*tempVal/(parmVec[5]^2)*(1/parmVec[1])*((exp(-tempVal/parmVec[5])-1)^(-1)))
        }else{

          ## ask Yan for her derivative derivation--change 0,0 part
          ## note below derivative w.r.t c and d are only for f==1 case!!!
          lEDder <-
            c(-log(exp(-tempVal/parmVec[5])-1)/(parmVec[1]^2),
              tempVal1/(parmVec[1]*(parmVec[3]-parmVec[2])),
              100/(parmVec[1]*(parmVec[3]-parmVec[2])*p),
              1/parmVec[4],
